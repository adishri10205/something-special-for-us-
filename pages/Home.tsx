      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-6 pb-20">
        {visibleCards.map((card, index) => (
          ...
          >
            <Link 
              to={card.to}
              className={`block h-36 md:h-48 rounded-[1.5rem] md:rounded-[2rem] bg-gradient-to-br ${card.gradient} shadow-lg ${card.shadow} relative overflow-hidden group`}
            >
              {/* Glassy Texture Overlay */}
              <div className="absolute inset-0 bg-white/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
              
              {/* Decorative Big Icon in Background */}
              <div className="absolute -bottom-4 -right-4 text-white opacity-20 transform rotate-12 scale-100 group-hover:scale-110 group-hover:rotate-6 transition-all duration-700">
